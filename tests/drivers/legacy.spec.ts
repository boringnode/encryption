...
const SECRET2 = 'anotherlongrandom32charactersstring'

test.group('Legacy', () => {
  test('fail when secret is missing', ({ assert }) => {
    assert.throws(
      // @ts-expect-error
      () => new Legacy({ keys: [null] }),
  ...
  test('decrypt encrypted value with multiple keys', ({ assert }) => {
    const driver = new Legacy({ keys: [SECRET2] })
    const driverWithKeyRotation = new Legacy({ keys: [SECRET, SECRET2] })
    const encrypted = driver.encrypt({ username: 'virk' })

    assert.deepEqual(driverWithKeyRotation.decrypt(encrypted), { username: 'virk' })
  })
